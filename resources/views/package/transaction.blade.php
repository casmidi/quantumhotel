...
form.addEventListener('keydown', function(event){if(event.key!=='Enter'){return;} const target=event.target; const managedField=target===mejaField || target===expiredDisplayField || target.classList.contains('item-code') || target.classList.contains('item-qty'); if(!managedField){return;} event.preventDefault(); if(target===expiredDisplayField){const normalizedExpired=normalizeDisplayDate(expiredDisplayField.value); if(!normalizedExpired){window.showCrudNotice('Expired date must use format dd-MM-yyyy.', 'Invalid Date'); expiredDisplayField.focus(); return;} if(!isNotPastDate(normalizedExpired)){window.showCrudNotice('Expired date must be greater than or equal to today.', 'Invalid Date'); expiredDisplayField.focus(); return;} expiredField.value=normalizedExpired; expiredDisplayField.value=formatDisplayDate(normalizedExpired);} if(focusNextEnterField(target)){return;} submitFromEnter();});

form.addEventListener('submit', function(event){mejaField.value=(mejaField.value||'').toString().trim().toUpperCase(); const normalizedExpired=normalizeDisplayDate(expiredDisplayField.value); if(!normalizedExpired){event.preventDefault(); window.showCrudNotice('Expired date must use format dd-MM-yyyy.', 'Invalid Date'); expiredDisplayField.focus(); return;} if(!isNotPastDate(normalizedExpired)){event.preventDefault(); window.showCrudNotice('Expired date must be greater than or equal to today.', 'Invalid Date'); expiredDisplayField.focus(); return;} expiredField.value=normalizedExpired; getRows().forEach((row)=>{const priceField=row.querySelector('.item-price'); if(priceField){priceField.value=unformat(priceField.value);} const qtyField=row.querySelector('.item-qty'); if(qtyField&&qtyField.value.trim()===''){qtyField.value='1';}});});

expiredDisplayField.addEventListener('blur', function(){if(!this.value){return;} const normalizedExpired=normalizeDisplayDate(this.value); if(!normalizedExpired){window.showCrudNotice('Expired date must use format dd-MM-yyyy.', 'Invalid Date'); this.focus(); return;} if(!isNotPastDate(normalizedExpired)){window.showCrudNotice('Expired date must be greater than or equal to today.', 'Invalid Date'); this.value=formatDisplayDate(expiredField.value || todayIso); this.focus(); return;} expiredField.value=normalizedExpired; this.value=formatDisplayDate(normalizedExpired);});
...
expiredNativeField.addEventListener('change', function(){if(!this.value){return;} if(!isNotPastDate(this.value)){window.showCrudNotice('Expired date must be greater than or equal to today.', 'Invalid Date'); this.value=expiredField.value || todayIso; expiredDisplayField.value=formatDisplayDate(this.value); return;} expiredField.value=this.value; expiredDisplayField.value=formatDisplayDate(this.value);});
