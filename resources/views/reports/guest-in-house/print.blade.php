...
<head>
    <meta charset="utf-8">
    <title>Guest In House - {{ \Carbon\Carbon::parse($businessDate)->format('Ymd') }}</title>
...
<div class="page">
    <div class="toolbar">
        <a href="/guest-in-house?business_date={{ $businessDate }}@if($search !== '')&search={{ urlencode($search) }}@endif" class="btn">Back</a>
    ...
    <article class="report-shell">
        <header class="hotel-header">
            ...
            <div class="report-meta">
                <div>Business Date: {{ \Carbon\Carbon::parse($businessDate)->format('d M Y') }}</div>
        ...
        <section class="report-title">
            <div>
                <h1>Guest In House Report</h1>
                <div class="muted">Official front office house count list for night audit reconciliation.</div>
            </div>
            <div class="muted">Format: DATA2 / ROOM</div>
        </section>

        <section class="summary-grid">
            <div class="summary-item"><small>Rooms</small><strong>{{ $number($summary['rooms']) }}</strong></div>
            <div class="summary-item"><small>Guest Rows</small><strong>{{ $number($summary['total_rows']) }}</strong></div>
            <div class="summary-item"><small>In House</small><strong>{{ $number($summary['in_house']) }}</strong></div>
            <div class="summary-item"><small>Checked Out</small><strong>{{ $number($summary['checked_out']) }}</strong></div>
                ...
                <tbody>
                    @forelse($rows as $row)
                        <tr>
                            ...
                            <td>{{ $row->Usaha ?: '-' }}</td>
                            <td class="text-right nowrap">{{ $money($row->Rate1) }}</td>
                            <td>{{ $row->Remark ?: '-' }}</td>
                            <td class="text-right">{{ $number($row->Day) }}</td>
                            <td>{{ $row->KodeNegara ?: '-' }}</td>
                            <td class="nowrap">{{ $row->PST }}{{ $row->control_flag ? ' / ' . $row->control_flag : '' }}</td>
                            <td>{{ $row->Officer ?: '-' }}</td>
                            <td>{{ $row->Lantai ?: '-' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="16">Tidak ada data Guest In House untuk business date ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
